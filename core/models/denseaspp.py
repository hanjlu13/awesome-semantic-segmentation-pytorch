...
__all__ = [
    "DenseASPP",
    "get_denseaspp",
    "get_denseaspp_densenet121_citys",
    "get_denseaspp_densenet161_citys",
    "get_denseaspp_densenet169_citys",
    "get_denseaspp_densenet201_citys",
]
    ...
    def __init__(
        ...
        **kwargs
        ...
        if backbone == "densenet121":
            self.pretrained = dilated_densenet121(
                dilate_scale, pretrained=pretrained_base, **kwargs
            )
        elif backbone == "densenet161":
            self.pretrained = dilated_densenet161(
                dilate_scale, pretrained=pretrained_base, **kwargs
            )
        elif backbone == "densenet169":
            self.pretrained = dilated_densenet169(
                dilate_scale, pretrained=pretrained_base, **kwargs
            )
        elif backbone == "densenet201":
            self.pretrained = dilated_densenet201(
                dilate_scale, pretrained=pretrained_base, **kwargs
            )
        else:
...
class _DenseASPPHead(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.dense_aspp_block = _DenseASPPBlock(
            in_channels, 256, 64, norm_layer, norm_kwargs
        )
        self.block = nn.Sequential(
            nn.Dropout(0.1), nn.Conv2d(in_channels + 5 * 64, nclass, 1)
        )
...
class _DenseASPPBlock(nn.Module):
    def __init__(
        ...
        self.aspp_24 = _DenseASPPConv(
            in_channels + inter_channels2 * 4,
            inter_channels1,
            inter_channels2,
            24,
            0.1,
    ...
    **kwargs
    ...
    if pretrained:
        ...
        model.load_state_dict(
            torch.load(
                get_model_file(
                    "denseaspp_%s_%s" % (backbone, acronyms[dataset]), root=root
                ),
                map_location=device,
            )
        )
