...
from core.models.base_models import EESP, eespnet
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.project_l3 = nn.Sequential(
            nn.Dropout2d(0.1), nn.Conv2d(128, nclass, 1, bias=False)
        )
    ...
    def forward(self, x):
        ...
        if self.aux:
            ...
            auxout = F.interpolate(
                proj_merge_l3_bef_act, size, mode="bilinear", align_corners=True
            )
...
class _PSPModule(nn.Module):
    def __init__(
        self, in_channels, out_channels=1024, sizes=(1, 2, 4, 8), **kwargs
    ):
    ...
    **kwargs
    ...
    if pretrained:
        ...
        model.load_state_dict(
            torch.load(
                get_model_file(
                    "espnet_%s_%s" % (backbone, acronyms[dataset]), root=root
                ),
                map_location=device,
            )
        )
