...
class EncNetLoss(nn.CrossEntropyLoss):
    ...
    def forward(self, *inputs):
        ...
        if not self.se_loss and not self.aux:
            ...
            return dict(
                loss=loss1 + self.aux_weight * loss2 + self.se_weight * loss3
            )
...
class ICNetLoss(nn.CrossEntropyLoss):
    ...
    def forward(self, *inputs):
        ...
        return dict(
            loss=loss1 + loss2 * self.aux_weight + loss3 * self.aux_weight
        )
...
class MixSoftmaxCrossEntropyOHEMLoss(OhemCrossEntropy2d):
    def __init__(
        self, aux=False, aux_weight=0.4, weight=None, ignore_index=-1, **kwargs
    ):
    ...
    def _aux_forward(self, *inputs, **kwargs):
        *preds, target = tuple(inputs)
        ...
        loss = super(MixSoftmaxCrossEntropyOHEMLoss, self).forward(
            preds[0], target
        )
        for i in range(1, len(preds)):
