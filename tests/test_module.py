...
def testSyncBN():
    def _check_batchnorm_result(bn1, bn2, input, is_train, cuda=False):
        ...
        _assert_tensor_close(
            _find_bn(bn1).bias.grad.data, _find_bn(bn2).bias.grad.data
        )
        _assert_tensor_close(
            _find_bn(bn1).weight.grad.data, _find_bn(bn2).weight.grad.data
        )
        _assert_tensor_close(input1.grad.data, input2.grad.data)
        _assert_tensor_close(
            _find_bn(bn1).running_mean, _find_bn(bn2).running_mean
        )
    ...
    for i in range(10):
        ...
        _check_batchnorm_result(
            bn, sync_bn, torch.rand(16, 10, 16, 16).double(), True, cuda=True
        )
