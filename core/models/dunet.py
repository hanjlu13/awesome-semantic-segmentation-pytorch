...
__all__ = [
    "DUNet",
    "get_dunet",
    "get_dunet_resnet50_pascal_voc",
    "get_dunet_resnet101_pascal_voc",
    "get_dunet_resnet152_pascal_voc",
]
    ...
    def __init__(
        ...
        **kwargs
        ...
        if aux:
            ...
            self.aux_dupsample = DUpsampling(
                256, nclass, scale_factor=8, **kwargs
            )

        self.__setattr__(
            "exclusive",
            ["dupsample", "head", "auxlayer", "aux_dupsample"]
            if aux
            else ["dupsample", "head"],
        )
...
class FeatureFused(nn.Module):
    ...
    def forward(self, c2, c3, c4):
        size = c4.size()[2:]
        c2 = self.conv2(
            F.interpolate(c2, size, mode="bilinear", align_corners=True)
        )
        c3 = self.conv3(
            F.interpolate(c3, size, mode="bilinear", align_corners=True)
        )
    ...
    **kwargs
    ...
    if pretrained:
        ...
        model.load_state_dict(
            torch.load(
                get_model_file(
                    "dunet_%s_%s" % (backbone, acronyms[dataset]), root=root
                ),
                map_location=device,
            )
        )
