...
class Downsampling(nn.Module):
    def __init__(self, in_channels, out_channels, **kwargs):
        ...
        self.conv1 = nn.Conv2d(
            in_channels, out_channels // 2, 3, 2, 2, bias=False
        )
...
class SSnbt(nn.Module):
    def __init__(
        self, in_channels, dilation=1, norm_layer=nn.BatchNorm2d, **kwargs
    ):
...
class APNModule(nn.Module):
    def __init__(
        self, in_channels, nclass, norm_layer=nn.BatchNorm2d, **kwargs
    ):
        super(APNModule, self).__init__()
        self.conv1 = _ConvBNReLU(
            in_channels, in_channels, 3, 2, 1, norm_layer=norm_layer
        )
        self.conv2 = _ConvBNReLU(
            in_channels, in_channels, 5, 2, 2, norm_layer=norm_layer
        )
        self.conv3 = _ConvBNReLU(
            in_channels, in_channels, 7, 2, 3, norm_layer=norm_layer
        )
        ...
        self.level5 = nn.Sequential(
            nn.AdaptiveAvgPool2d(1), _ConvBNReLU(in_channels, nclass, 1)
        )
    ...
    **kwargs
    ...
    if pretrained:
        ...
        model.load_state_dict(
            torch.load(
                get_model_file("lednet_%s" % (acronyms[dataset]), root=root),
                map_location=device,
            )
        )
    return model
