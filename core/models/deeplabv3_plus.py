    ...
    def __init__(
        ...
        **kwargs
        ...
        self.pretrained = get_xception(
            pretrained=pretrained_base, output_stride=output_stride, **kwargs
        )
...
class _DeepLabHead(nn.Module):
    def __init__(
        self, nclass, c1_channels=128, norm_layer=nn.BatchNorm2d, **kwargs
    ):
        super(_DeepLabHead, self).__init__()
        self.aspp = _ASPP(2048, [12, 24, 36], norm_layer=norm_layer, **kwargs)
        self.c1_block = _ConvBNReLU(
            c1_channels, 48, 3, padding=1, norm_layer=norm_layer
        )
    ...
    **kwargs
    ...
    if pretrained:
        ...
        model.load_state_dict(
            torch.load(
                get_model_file(
                    "deeplabv3_plus_%s_%s" % (backbone, acronyms[dataset]),
                    root=root,
                ),
                map_location=device,
            )
