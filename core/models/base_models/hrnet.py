...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = nn.Conv2d(
            inplanes, planes, 3, stride, padding=1, bias=False
        )
        ...
        for i in range(num_branches):
            branches.append(
                self._make_one_branch(
                    i, block, num_blocks, num_channels, norm_layer=norm_layer
                )
            )
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.layer1 = self._make_layer(
            Bottleneck, 64, 64, 4, norm_layer=norm_layer
        )
        ...
        channels = [channel * block.expansion for channel in num_channel]
        self.transition1 = self._make_transition_layer(
            pre_stage_channels, channels, norm_layer
        )
            ...
            if i < num_branches_pre:
                ...
                for j in range(i + 1 - num_branches_pre):
                    ...
                    conv3x3s.append(
                        nn.Sequential(
                            nn.Conv2d(
                                in_channels, out_channels, 3, 2, 1, bias=False
                            ),
    ...
    def _make_head(self, pre_stage_channels, norm_layer=nn.BatchNorm2d):
        ...
        for i, channels in enumerate(pre_stage_channels):
            incre_module = self._make_layer(
                head_block, channels, head_channels[i], 1
            )
...
if __name__ == "__main__":
    ...
    model = HighResolutionNet(
        blocks, num_channels, num_modules, num_branches, num_blocks, fuse_method
    )
    output = model(img)
