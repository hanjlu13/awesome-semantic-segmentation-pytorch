...
__all__ = [
    ...
    "get_fcn16s",
    "get_fcn8s",
    "get_fcn32s_vgg16_voc",
    "get_fcn16s_vgg16_voc",
    "get_fcn8s_vgg16_voc",
]
...
class FCN16s(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.__setattr__(
            "exclusive",
            ["head", "score_pool4", "auxlayer"]
            if aux
            else ["head", "score_pool4"],
        )
    ...
    def forward(self, x):
        ...
        out = F.interpolate(
            fuse_pool4, x.size()[2:], mode="bilinear", align_corners=True
        )
        outputs.append(out)
        ...
        if self.aux:
            auxout = self.auxlayer(pool5)
            auxout = F.interpolate(
                auxout, x.size()[2:], mode="bilinear", align_corners=True
            )
            outputs.append(auxout)
...
class FCN8s(nn.Module):
    ...
    def forward(self, x):
        ...
        out = F.interpolate(
            fuse_pool3, x.size()[2:], mode="bilinear", align_corners=True
        )
        outputs.append(out)
    ...
    **kwargs
    ...
    if pretrained:
        ...
        model.load_state_dict(
            torch.load(
                get_model_file(
                    "fcn32s_%s_%s" % (backbone, acronyms[dataset]), root=root
                ),
                map_location=device,
            )
        )
    ...
    **kwargs
    ...
    if pretrained:
        ...
        model.load_state_dict(
            torch.load(
                get_model_file(
                    "fcn16s_%s_%s" % (backbone, acronyms[dataset]), root=root
                ),
                map_location=device,
            )
        )
    ...
    **kwargs
    ...
    if pretrained:
        ...
        model.load_state_dict(
            torch.load(
                get_model_file(
                    "fcn8s_%s_%s" % (backbone, acronyms[dataset]), root=root
                ),
                map_location=device,
            )
        )
