...
class _DenseLayer(nn.Sequential):
    def __init__(
        ...
        self.add_module(
            "conv1",
            nn.Conv2d(
                num_input_features, bn_size * growth_rate, 1, 1, bias=False
            ),
        ),
...
class _Transition(nn.Sequential):
    def __init__(
        self, num_input_features, num_output_features, norm_layer=nn.BatchNorm2d
    ):
        ...
        self.add_module(
            "conv",
            nn.Conv2d(
                num_input_features, num_output_features, 1, 1, bias=False
            ),
        )
...
class DilatedDenseNet(DenseNet):
    def __init__(
        ...
        **kwargs
        ...
        assert (
            dilate_scale == 8 or dilate_scale == 16
        ), "dilate_scale can only set as 8 or 16"
        from functools import partial

        if dilate_scale == 8:
            self.features.denseblock3.apply(
                partial(self._conv_dilate, dilate=2)
            )
            self.features.denseblock4.apply(
                partial(self._conv_dilate, dilate=4)
            )
...
def get_dilated_densenet(num_layers, dilate_scale, pretrained=False, **kwargs):
    ...
    model = DilatedDenseNet(
        growth_rate, block_config, num_init_features, dilate_scale=dilate_scale
    )
    if pretrained:
        # '.'s are no longer allowed in module names, but pervious _DenseLayer
        # has keys 'norm.1', 'relu.1', 'conv.1', 'norm.2', 'relu.2', 'conv.2'.
